...
namespace ReCORE.ReCore.Core.Spells
{
    class Smite : ISpell
    {
        ...
        public void Execute()
        {
            if (SummnerManager.Smite.Handle.Ammo <= MenuHelper.GetSliderValue(Summoners.Menu, "Summoners.Smite.Keep.Count")) return;
            ...
            if (Monster != null)
                SummnerManager.Smite.Cast(Monster);
            ...
            {
                SummnerManager.SmiteType smiteType = SummnerManager.SmiteType.None;
                switch (SummnerManager.Smite.Handle.Name)
                {
                    case "s5_SummonerSmitePlayerGanker": smiteType = SummnerManager.SmiteType.Chilling; break;
                    case "s5_SummonerSmiteDuel": smiteType = SummnerManager.SmiteType.Challenging; break;
                }
                if (smiteType != SummnerManager.SmiteType.None)
                {
                    Obj_AI_Base ks = EloBuddy.SDK.EntityManager.Heroes.Enemies.FirstOrDefault(p =>
                                        ...
                                        p.IsValidTarget(SummnerManager.Smite.Range)
                                        );
                    if (ks != null)
                        SummnerManager.Smite.Cast(ks);
                }
            }

            if (SummnerManager.Smite.Handle.Ammo <= MenuHelper.GetSliderValue(Summoners.Menu, "Summoners.Smite.Keep.Count")) return;

            Obj_AI_Base target = TargetSelector.GetTarget(SummnerManager.Smite.Range, DamageType.Mixed);
            ...
            switch (SummnerManager.Smite.Handle.Name)
            {
                case "S5_SummonerSmitePlayerGanker": // Blue Smite
                    if (target.HealthPercent <= Summoners.Menu.GetSliderValue("Summoners.Smite.Champions.Health") && target.IsInAutoAttackRange(Player.Instance))
                        SummnerManager.Smite.Cast(target);
                    break;
                case "S5_SummonerSmiteDuel": // Red Smite
                    if (target.HealthPercent <= Summoners.Menu.GetSliderValue("Summoners.Smite.Champions.Health") && target.IsInRange(Player.Instance, SummnerManager.Smite.Range / 2))
                        SummnerManager.Smite.Cast(target);
                    break;
                default:
                    break;
        ...
        {
            if (!SummnerManager.Smite.IsReady() || SummnerManager.Smite.IsOnCooldown || !MenuHelper.GetKeyBindValue(Summoners.Menu, "Summoners.Smite.Keybind"))
                return false;
            return true;
        }
        ...
        public void OnDraw()
        {
            ...
            {
                Circle.Draw(SharpDX.Color.Blue, SummnerManager.Smite.Range, Player.Instance);
            }
            if (Summoners.Menu.GetCheckBoxValue("Summoners.Smite.Draw.Status"))
            {
                ...
                int charges = SummnerManager.Smite.Handle.Ammo;
        ...
        {
            if (Summoners.Menu.GetCheckBoxValue("Summoners.Smite.Draw.Damage") && SummnerManager.Smite.IsReady() && !SummnerManager.Smite.IsOnCooldown)
            {
                foreach (var unit in EloBuddy.SDK.EntityManager.MinionsAndMonsters.GetJungleMonsters().Where(m => m.IsHPBarRendered && !m.Name.Contains("Mini") && m.IsValidTarget(SummnerManager.Smite.Range + 350)))
                {
                    #region Monsters
